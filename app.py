...
def install_requirements(bot_id):
    ...
    if not os.path.exists(req_file):
        return jsonify({"success": False, "error": "requirements.txt not found"}), 404

    install_log_path = os.path.join(LOGS_DIR, f"{bot_id}_install.log")

    try:
        # Decrypt requirements.txt first
        success, content = read_file_decrypted(bot_id, "requirements.txt")
        if not success:
            return jsonify({"success": False, "error": "Failed to read requirements.txt"}), 500
        
        temp_req = os.path.join("bots_data/temp", f"{bot_id}_requirements.txt")
